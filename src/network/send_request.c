...
void _set_ip_header(struct iphdr* ip_header, struct sockaddr_in* destination, u8 ttl) {
    ...
    // ip_header->tot_len  = htons(g_outpacket_info.m_size);   /* Total Length */
...
//     udp_header->source  = htons(g_arguments.m_options.m_src_port);  /* Source Port: our PxID */
...
FT_RESULT send_request(const u8 ttl) {
    ...
    g_outpacket_info.m_size = g_arguments.m_options.m_packet_size;
